...
class FirebaseUserRepository implements UserRepository {
  ...
  Future<Result<User>> updateUserBalance(
    ...
    if (result.exists) {
      ...
      // * MENGAMBIL DATA SNAPSHOT BARU
      DocumentSnapshot<Map<String, dynamic>> updateResult =
          await documentReference.get();

      if (updateResult.exists) {
        // * MEMBANDINGKAN BALANCE YANG ADA DI FIREBASE DAN BALANCE SAAT INI
        User updateUser = User.fromJson(updateResult.data()!);
        if (updateUser.balance == balance) {
          // * JIKA HASIL PEMBANDINGAN SAMA
          return Result.success(updateUser);
        } else {
          // * JIKA HASIL PEMBANDINGAN TIDAK SAMA
  ...
  Future<Result<User>> uploadProfilePicture(
      {required User user, required File imageProfile}) async {
